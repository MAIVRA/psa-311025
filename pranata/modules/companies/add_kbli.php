<?php
// modules/companies/add_kbli.php
...
if (empty($company_id)) {
    $_SESSION['flash_message'] = "Error: ID Perusahaan tidak valid untuk menambah KBLI.";
    // === [PERBAIKAN PATH: MENGGUNAKAN BASE_URL] ===
...
// Set judul halaman
$page_title = "Tambah KBLI untuk " . htmlspecialchars($company_name);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $kode_kbli = trim($_POST['kode_kbli']);
        $deskripsi = !empty($_POST['deskripsi']) ? trim($_POST['deskripsi']) : NULL;

        // Validasi Sederhana
        if (empty($kode_kbli)) {
            throw new Exception("Kode KBLI wajib diisi.");
        }

        // Insert ke tabel company_kbli
        $sql_insert = "INSERT INTO company_kbli (company_id, kode_kbli, deskripsi) VALUES (?, ?, ?)";
        ...
        // Tipe data: i s s
        $stmt_insert->bind_param("iss",
            $company_id, $kode_kbli, $deskripsi
        );

        if (!$stmt_insert->execute()) {
             throw new Exception("Gagal menyimpan data KBLI: " . $stmt_insert->error);
        }
        $stmt_insert->close();
        $conn_db->close();

        // Set flash message dan redirect kembali ke view company
        $_SESSION['flash_message'] = "KBLI '" . htmlspecialchars($kode_kbli) . "' berhasil ditambahkan!";
        // === [PERBAIKAN PATH: MENGGUNAKAN BASE_URL] ===
...
<main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-6">
    ...
    <form action="<?php echo BASE_URL; ?>/modules/companies/add_kbli.php?company_id=<?php echo $company_id; ?>" method="POST" class="bg-white p-6 rounded-lg shadow-md space-y-6 max-w-xl mx-auto border border-gray-200">
    <div>
            <label for="kode_kbli" class="form-label">Kode KBLI <span class="text-red-500">*</span></label>
            <input type="text" id="kode_kbli" name="kode_kbli" class="form-input" required placeholder="Contoh: 70201 (Aktivitas Konsultasi Manajemen)">
            <p class="text-xs text-gray-500 mt-1">Masukkan kode KBLI (biasanya 5 digit).</p>
        </div>

        <div>
            <label for="deskripsi" class="form-label">Deskripsi KBLI (Opsional)</label>
            <textarea id="deskripsi" name="deskripsi" rows="3" class="form-input" placeholder="Masukkan deskripsi singkat bidang usaha sesuai kode KBLI"></textarea>
             <p class="text-xs text-gray-500 mt-1">Anda bisa mengisi deskripsi untuk memperjelas.</p>
        </div>
